    ...
    public function crearEmpleado(Request $request)
    {
        ...
        try {
            ...
            $empleadoExistente = Empleado::where('s_nombre', $request->s_nombre)
                ...
                ->whereDate('d_fecha_nacimiento', $request->d_fecha_nacimiento)
                ->first();
            ...
            $telefonoEmergencia = $request->s_telefono_contacto_emergencia
                ? preg_replace('/[^0-9]/', '', $request->s_telefono_contacto_emergencia)
                : null;
            ...
            $empleado->fill([
                ...
                's_telefono' => $telefono,
                's_telefono_contacto_emergencia' => $telefonoEmergencia,
    ...
    public function actualizarEmpleado(Request $request, $id_empleado)
    {
        try {
            ...
            if ($request->has('s_telefono')) {
                $empleado->s_telefono = preg_replace(
                    '/[^0-9]/',
                    '',
                    $request->s_telefono
                );
            ...
            if ($request->has('s_telefono_contacto_emergencia')) {
                $empleado->s_telefono_contacto_emergencia = preg_replace(
                    '/[^0-9]/',
                    '',
                    $request->s_telefono_contacto_emergencia
                );
            }
            ...
            if ($request->has('s_foto_empleado')) {
                if ($request->s_foto_empleado === null) {
                    ...
                    if (
                        $empleado->s_foto_empleado &&
                        $empleado->s_foto_empleado !== 'empleado-default.png'
                    ) {
                        $this->deleteImage($empleado->s_foto_empleado);
                    }

                    $empleado->s_foto_empleado = $this->saveBase64Image(
                        $request->s_foto_empleado,
                        'empleado_' . $id_empleado . '_'
                    );
                }
            }
            ...
            // Cambió tipo → actualizar habilidades
            if (
                $request->has('id_tipo_empleado') &&
                $request->id_tipo_empleado != $tipo_anterior
            ) {
                $this->actualizarHabilidadesPorTipo(
                    $id_empleado,
                    $request->id_tipo_empleado
                );
            }
